-- AWESOME CONFIG 
-- Author John MacTavish (john@techsolutionsx.com)
-- Derived from the standard file for Awesome >= 3.5
...
-- beautiful.init(configpath .. "/themes/" ..theme.. "/awesome-solarized/dark/theme.lua")
...
if screen.count() == 1 then
  tags1_1 = {
    names  = { "[main]",   "[shell]", "[code]",    "[www]",    "[mail/cal]", "[remote]", "[other]" },
    layout = { layouts[1], layouts[3], layouts[2], layouts[1], layouts[2], layouts[6], layouts[3] }
  }
...
-- {{{ Wibox
-- Widget definition and registration go here.
-- Initialize widget, use widget({ type = "textbox" }) for awesome < 3.5 then
-- Register widget with vicious.register(widgetname, then options from http://git.sysphere.org/vicious/tree/README)
...
-- And of course the clssic Create a textclock widget
...
for s = 1, screen.count() do
  ...
  -- add additional ones here, each with it's own line for example
  -- left_layout:add(yourwidgetname) note that order matters
  ...
  -- add additional ones here, each with it's own line for example
  -- right_layout:add(yourwidgetname) note that order matters
...
awful.key({ }, "XF86AudioRaiseVolume", function ()
   awful.util.spawn("/usr/bin/amixer set Master 9%+") end),
awful.key({ }, "XF86AudioLowerVolume", function ()
   awful.util.spawn("/usr/bin/amixer set Master 9%-") end),
awful.key({ }, "XF86AudioMute", function ()
       awful.util.spawn("/usr/bin/amixer sset Master toggle") end),
